...
modify.on('modifyend', e => {
    ...
    for(var i = 0; i < features.length; i++) {
        ...
        if(rev > 1) {
            let created = JSON.parse((new ol.format.GeoJSON()).writeFeature(features[i], {
                featureProjection : 'EPSG:3857'
            }))
            let layer 
            for(let f of range.getFeatures()) {
            ...
            let diff = turf.buffer(turf.difference(created, layer), 0)
            let geom = (new ol.format.GeoJSON()).readFeature(diff, {
                featureProjection: 'EPSG:3857'
            })
            features[i].setGeometry(geom.getGeometry())
        }
    }
})
*/
